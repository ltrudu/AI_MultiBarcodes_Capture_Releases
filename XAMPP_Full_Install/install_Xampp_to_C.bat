...
echo =========================================================
echo XAMPP Automated Installer - Install to C:\xampp
echo =========================================================
echo.
...
REM Check if C:\xampp exists
if exist "C:\xampp" (
    echo [WARNING] C:\xampp already exists!
...
echo [STEP 3] Extracting to C:\xampp...
echo This may take several minutes...
echo.

REM Try tar first (Windows 10+)
tar -xf "archive\xampp.zip" -C "C:\" 2>nul

if errorlevel 1 (
    echo [INFO] tar failed, trying PowerShell...
    powershell -NoProfile -ExecutionPolicy Bypass -Command "$shell=New-Object -COM Shell.Application; $zip=$shell.NameSpace('%CD%\archive\xampp.zip'); $dest=$shell.NameSpace('C:\'); if($zip){ $dest.CopyHere($zip.Items(),16) }else{ exit 1 }"

    if errorlevel 1 (
        echo [ERROR] Extraction failed!
        echo Please extract manually:
        echo 1. Right-click archive\xampp.zip
        echo 2. Extract All to C:\
        if "%JOINED_ARCHIVE%"=="1" del /Q "archive\xampp.zip" 2>nul
        pause
        exit /b 1
    )
...
echo.
echo [OK] C:\xampp\apache\bin\httpd.exe
echo [OK] C:\xampp\mysql\bin\mysqld.exe
echo [OK] C:\xampp\php\php.exe
echo.
